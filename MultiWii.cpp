...
const char boxnames[] PROGMEM = // names for dynamic generation of config GUI
  ...
  "LAND;"
#endif
//for any VTOL aircraft, we init these 3 types of mode
#if defined(VTOL)
  "HOV;"
  "SFF;"
  "FFF;"
...
//LFP Telemetry
  uint8_t LFP_telemetry = 0;
  unsigned long LFP_last, LFP_now;
...
void loop () {
  ...
  if ((int16_t)(currentTime-rcTime) >0 ) { // 50Hz
    ...
    if(rcDelayCommand == 20) {
      if(f.ARMED) {                   // actions during armed
        ...
        #if defined(LFP_TELEMETRY) 
          //Lazeman's LFP telemetry swtich
          else if (rcSticks == THR_LO + YAW_CE + PIT_HI + ROL_LO) {              // Auto telemetry ON/OFF
            if (LFP_telemetry) {
              SerialOpen(LFP_TELEMETRY_PORT,SERIAL0_COM_SPEED); //back to original speed
              LFP_telemetry = 0;
            } else
            {
              SerialOpen(LFP_TELEMETRY_PORT,LFP_TELEMETRY_SPEED);//init with telemetry speed
              LFP_telemetry = 1;
            }
          }
        #endif
    ...
    else if(rcOptions[BOXSFF]) 
    {
      f.HOV_MODE = 0;
      f.SFF_MODE = 1;
      f.FFF_MODE = 0;
    }
    else if(rcOptions[BOXFFF]) 
    {
      f.HOV_MODE = 0;
      f.SFF_MODE = 0;
      f.FFF_MODE = 1;
  ...
  */
  ...
  #if defined(LFP_TELEMETRY) 
  //Lazeman's LFP telemetry
  if(LFP_telemetry)
  {
    LFP_now = millis();
    if(LFP_now - LFP_last > LFP_PERIOD)
    {
      LFP_last = LFP_now;
      run_LFP_telemetry();
    }
