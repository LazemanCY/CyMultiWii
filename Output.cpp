...
#if defined(VTOL_TR)
void ServoSlowTrans(int16_t Target)
{
  if(servo[2] > Target)
  {
    servo[2] -= TRANS_SERVO_STEP;
    if(servo[2] < Target)
      servo[2] = Target;
  }
  else if(servo[2] < Target)
  {
    servo[2] += TRANS_SERVO_STEP;
    if(servo[2] > Target)
      servo[2] = Target;
...
void writeMotors() { // [1000;2000] => [125;250]
  /****************  Specific PWM Timers & Registers for the MEGA's   *******************/
    ...
    #if (NUMBER_MOTOR > 4)
      #ifndef EXT_MOTOR_RANGE 
        OCR4B = motor[4]<<3; //  pin 7  //VTOL trust motor
...
void initializeServo() {
  ...
  // do pins init
    #if (PRI_SERVO_FROM == 1) || (SEC_SERVO_FROM == 1)
      SERVO_1_PINMODE;  //VTOL init servo 1
    #endif
    #if (PRI_SERVO_FROM <= 2 && PRI_SERVO_TO >= 2) || (SEC_SERVO_FROM <= 2 && SEC_SERVO_TO >= 2) 
      SERVO_2_PINMODE; //VTOL init servo 2
    #endif
    #if (PRI_SERVO_FROM <= 3 && PRI_SERVO_TO >= 3) || (SEC_SERVO_FROM <= 3 && SEC_SERVO_TO >= 3) 
      SERVO_3_PINMODE;
    #endif 
...
void mixTable() {
  ...
  /****************                   main Mix Table                ******************/
    ...
    // Traditional Flaps on SERVO3
    ...
    #if defined(AIRPLANE_VTAIL) //For V-tail we have different servo movement mix
      if(f.PASSTHRU_MODE){   // Direct passthru from RX
        ...
        servo[5] = rcCommand[YAW] + axisPID[PITCH];                    //   Rudder
        servo[6] = rcCommand[YAW] - axisPID[PITCH];                //   Elevator
      }
    #else
      if(f.PASSTHRU_MODE){   // Direct passthru from RX
        servo[3] = rcCommand[ROLL] + flapperons[0];     //   Wing 1
  ...
  /*****************************               VTOL_SLT/TR               **************************************/  
  #elif defined( VTOL_SLT ) || defined(VTOL_TR)
    //悬停模式下，升力电机按照标准四旋翼的模式进行反馈控制，推力电机关闭
    if(f.HOV_MODE)
    {
      motor[0] = PIDMIX(-1,+1,-1); //REAR_R
      motor[1] = PIDMIX(-1,-1,+1); //FRONT_R
      ...
      servo[0] = rcCommand[ROLL];
      servo[1] = rcCommand[YAW];
      servo[3] = rcCommand[PITCH];    
      
      #if defined(VTOL_SLT)
        servo[2] = MINCOMMAND;
      #elif defined(VTOL_TR)
        ServoSlowTrans(TRANS_SERVO_HOV);
        //servo[2] = TRANS_SERVO_HOV;
      #endif
    }
    //慢速飞行模式下，升力电机工作方式和悬停模式相同，推力电机半油门推进
    else if(f.SFF_MODE)
    {
      motor[0] = PIDMIX(-1,+1,-1); //REAR_R
      motor[1] = PIDMIX(-1,-1,+1); //FRONT_R
      ...
      servo[3] = rcCommand[PITCH];  
        ...
        servo[2] = SFF_THR;
      #elif defined(VTOL_TR)
        ServoSlowTrans(TRANS_SERVO_SFF);
        //servo[2] = TRANS_SERVO_SFF;
      #endif
    }
    //快速飞行模式下，升力电机关闭，推力电机按照油门杆量输出
    else if(f.FFF_MODE)
    {
      #if defined(VTOL_SLT)
      ...
      #elif defined(VTOL_TR)
        motor[0] = constrain(rcCommand[THROTTLE], MINCOMMAND, FFF_MAXTHROTTLE);
        motor[1] = constrain(rcCommand[THROTTLE], MINCOMMAND, FFF_MAXTHROTTLE);
        motor[2] = constrain(rcCommand[THROTTLE], MINCOMMAND, FFF_MAXTHROTTLE);
        motor[3] = constrain(rcCommand[THROTTLE], MINCOMMAND, FFF_MAXTHROTTLE);
      #endif
      ...
      #if defined(VTOL_SLT)
        servo[2] = constrain(rcCommand[THROTTLE], MINCOMMAND, FFF_MAXTHROTTLE);
      #elif defined(VTOL_TR)
        ServoSlowTrans(TRANS_SERVO_FFF);
        //servo[2] = TRANS_SERVO_FFF;
      #endif
    }
    
    //为了能够使用地面站进行中立点配置，这里化用了原本固定翼的设置
    //ail servo use servo[4] setting
    servo[0]  = ((int32_t)conf.servoConf[4].rate * servo[0])/100L;
    servo[0] += get_middle(4);
    //rudder servo use servo[5] setting
    servo[1]  = ((int32_t)conf.servoConf[5].rate * servo[1])/100L;
    servo[1] += get_middle(5);
    //ele servo use servo[3] setting
    servo[3]  = ((int32_t)conf.servoConf[3].rate * servo[3])/100L; 
    servo[3] += get_middle(3);
    
    #if defined(VTOL_SLT)
    if(!f.ARMED) 
      servo[2] = MINCOMMAND; //kill thrust motor when disarmed
    #endif
      
    //in order to use airplane setting and display in GUI, we map servo[0] -> servo[4], servo[1] -> servo[5]
    servo[4] = servo[0];
    servo[5] = servo[1];  
    servo[6] = servo[2];
    
    //as we use airplane type in GUI:
    //GUI中显示的舵机位置和VTOL实际使用的舵机的对应关系
    /*
              GUI       VTOL
    SERVO[3]  Ail1      ELE
    SERVO[4]  Ail2      AIL
    SERVO[5]  Rud       Rud
    SERVO[6]  Ele    Thrust Motor
    */
