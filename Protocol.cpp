...
void evaluateCommand(uint8_t c) {
  ...
  switch(c) {
    ...
    case MSP_STATUS:
      ...
      #endif
      #if defined(VTOL)
        if(f.HOV_MODE) tmp |= 1<<BOXHOV;
        if(f.SFF_MODE) tmp |= 1<<BOXSFF;
        if(f.FFF_MODE) tmp |= 1<<BOXFFF;
      #endif
      #if defined(BUZZER)
...
void run_LFP_telemetry(void)
{
  ...
  */
  ...
  CURRENTPORT = LFP_TELEMETRY_PORT;
  
  if(f.ARMED)         flight_mode |= 1<<7;
  if(f.ANGLE_MODE)    flight_mode |= 1<<6;
  if(f.HORIZON_MODE)  flight_mode |= 1<<5;
  if(f.MAG_MODE)      flight_mode |= 1<<4;
  if(f.BARO_MODE)     flight_mode |= 1<<3;
  #if defined(FIXEDWING) || defined(HELICOPTER)
    if(f.PASSTHRU_MODE) flight_mode |= 1<<2;
  #endif
  ...
  serialize16(0);             //used mah number  TBD
  serialize16(analog.rssi);   //rssi value
  serialize8(0);              //flight speed     TBD
  serialize16(att.angle[PITCH]);//angle_pit
  serialize16(att.angle[ROLL]);//angle_rol
  serialize16(att.heading);   //mag heading
  serialize16( (int16_t)(alt.EstAlt/10));//Alt in 0.1m scale
  serialize8(flight_mode);               //flight mode
  serialize8(0);             //reserved for VTOL type
  
  serialize8(checksum[LFP_TELEMETRY_PORT]);
  UartSendData(LFP_TELEMETRY_PORT);
}
#endif
